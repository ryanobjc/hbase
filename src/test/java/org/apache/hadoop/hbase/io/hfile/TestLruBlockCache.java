...
public class TestLruBlockCache extends TestCase {
  ...
  public void testCacheSimple() throws Exception {
    ...
    // Confirm empty
    for(Block block : blocks) {
      assertTrue(cache.getBlock(block.blockName, null) == null);
    }
    ...
    for(Block block : blocks) {
      ByteBuffer buf = cache.getBlock(block.blockName, null);
      assertTrue(buf != null);
  ...
  public void testCacheEvictionSimple() throws Exception {
    ...
    assertTrue(cache.getBlock(blocks[0].blockName, null) == null);
    assertTrue(cache.getBlock(blocks[1].blockName, null) == null);
    for(int i=2;i<blocks.length;i++) {
      assertEquals(cache.getBlock(blocks[i].blockName, null),
          blocks[i].buf);
    }
  }
  ...
  public void testCacheEvictionTwoPriorities() throws Exception {
    ...
    for(Block block : multiBlocks) {
      cache.cacheBlock(block.blockName, block.buf);
      expectedCacheSize += block.heapSize();
      assertEquals(cache.getBlock(block.blockName, null), block.buf);
    }
    ...
    assertTrue(cache.getBlock(singleBlocks[0].blockName, null) == null);
    assertTrue(cache.getBlock(multiBlocks[0].blockName, null) == null);

    // And all others to be cached
    for(int i=1;i<4;i++) {
	assertEquals(cache.getBlock(singleBlocks[i].blockName, null),
          singleBlocks[i].buf);
	assertEquals(cache.getBlock(multiBlocks[i].blockName, null),
          multiBlocks[i].buf);
    }
  }
  ...
  public void testCacheEvictionThreePriorities() throws Exception {
    ...
    for(int i=0;i<3;i++) {
      ...
      cache.getBlock(multiBlocks[i].blockName, null);
    ...
    assertEquals(null, cache.getBlock(singleBlocks[0].blockName, null));

    // Change the oldest remaining single block to a multi
    cache.getBlock(singleBlocks[1].blockName, null);
    ...
    assertEquals(null, cache.getBlock(multiBlocks[0].blockName, null));
    ...
    assertEquals(null, cache.getBlock(memoryBlocks[0].blockName, null));
    ...
    assertEquals(null, cache.getBlock(singleBlocks[2].blockName, null));
    assertEquals(null, cache.getBlock(singleBlocks[3].blockName, null));
    assertEquals(null, cache.getBlock(singleBlocks[4].blockName, null));

    // Make the big block a multi block
    cache.getBlock(bigBlocks[0].blockName, null);
    ...
    assertEquals(null, cache.getBlock(singleBlocks[1].blockName, null));
    assertEquals(null, cache.getBlock(multiBlocks[1].blockName, null));
    assertEquals(null, cache.getBlock(multiBlocks[2].blockName, null));
    ...
    assertEquals(null, cache.getBlock(memoryBlocks[1].blockName, null));
    assertEquals(null, cache.getBlock(memoryBlocks[2].blockName, null));
    assertEquals(null, cache.getBlock(memoryBlocks[3].blockName, null));
  ...
  public void testScanResistance() throws Exception {
    ...
    for(Block block : multiBlocks) {
      cache.cacheBlock(block.blockName, block.buf);
      cache.getBlock(block.blockName, null);
    }
  ...
  public void testResizeBlockCache() throws Exception {
    ...
    for(int i=0;i<5;i++) {
	assertEquals(null, cache.getBlock(singleBlocks[i].blockName, null));
	assertEquals(null, cache.getBlock(multiBlocks[i].blockName, null));
	assertEquals(null, cache.getBlock(memoryBlocks[i].blockName, null));
    }

    // And the newest 5 blocks should still be accessible
    for(int i=5;i<10;i++) {
	assertEquals(singleBlocks[i].buf, cache.getBlock(singleBlocks[i].blockName, null));
	assertEquals(multiBlocks[i].buf, cache.getBlock(multiBlocks[i].blockName, null));
	assertEquals(memoryBlocks[i].buf, cache.getBlock(memoryBlocks[i].blockName, null));
    }
  }
