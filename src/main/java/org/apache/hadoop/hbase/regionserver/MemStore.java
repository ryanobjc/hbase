  ...
  public long updateColumnValue(byte[] row,
   ...
   this.lock.readLock().lock();
    try {
      ...
      KeyValue targetKV = KeyValue.createFirstOnRow(row, family, qualifier);
      SortedSet<KeyValue> ss = kvset.tailSet(targetKV);
      ...
      while ( it.hasNext() ) {
        ...
        if (0 != Bytes.compareTo(
            targetKV.getBuffer(), targetKV.getRowOffset(), targetKV.getRowLength(),
        ...
        if (0 == Bytes.compareTo(
            targetKV.getBuffer(), targetKV.getQualifierOffset(), targetKV.getQualifierLength(),
